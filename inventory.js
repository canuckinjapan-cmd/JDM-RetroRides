...
function renderInventory() {
  ...
  filtered.sort((a, b) => {
    ...
    // Default Sort: Featured Order first, then Newest (createdAt) second
    const featA = Number(a.featured) || 999;
    const featB = Number(b.featured) || 999;
  ...
  if (filtered.length === 0) {
    vehicleContainer.innerHTML = `
      <div class="text-center py-20 space-y-4">
        <div class="text-slate-500 font-bold text-xl">No vehicles found matching your criteria.</div>
  ...
  const countSpan = document.getElementById('vehicle-count-span');
