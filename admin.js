  ...
  try {
    ...
    onSnapshot(q, async (snapshot) => {
      if (snapshot.empty && allVehiclesAdmin.length === 0) {
        if (window.logDebug) window.logDebug("⚠️ Snapshot empty, attempting server-side fetch...");
        try {
          const serverSnapshot = await getDocsFromServer(q);
          if (!serverSnapshot.empty) {
            allVehiclesAdmin = serverSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data(), metadata: doc.metadata }));
            renderAdminList();
            updateStatus(true, 'Live (Server)');
            return;
          }
        } catch (e) {
        ...
        <td class="px-4 py-4 md:px-6 block md:table-cell">
          <div class="flex items-center gap-3">
            <img src="${firstImage}" 
                 ...
                 onerror="this.onerror=null; this.src='https://placehold.co/100x100/1e293b/white?text=Car';">
...
function renderPhotoPreviews() {
  ...
  currentVehicleImages.forEach((url, index) => {
    ...
    div.innerHTML = `
      <img src="${url}" referrerpolicy="no-referrer" class="w-full h-full object-cover" onerror="this.src='https://placehold.co/200x200/1e293b/white?text=Broken+Link';">
...
restoreDataBtn.addEventListener('click', async () => {
  if (!confirm('This will CLEAR ALL existing listings and restore the 5 initial vehicle listings. Continue?')) return;
  ...
  try {
    ...
    if (!snapshot.empty) {
      ...
      if (window.logDebug) window.logDebug(`🗑️ Deleting ${snapshot.size} existing vehicles...`);
      const deleteBatch = writeBatch(db);
      snapshot.docs.forEach(vDoc => deleteBatch.delete(vDoc.ref));
      await withTimeoutAndRetry(() => deleteBatch.commit(), 'Delete Batch Commit');
      if (window.logDebug) window.logDebug("✅ Old data cleared.");
    }
    console.log('Cleared existing inventory');

    updateStatus(false, 'Seeding New Data...');
    if (!Array.isArray(initialVehicles) || initialVehicles.length === 0) {
      throw new Error("Initial data is missing or empty. Please check initialData.js.");
    }
    const reversedVehicles = [...initialVehicles].reverse();
    const createBatch = writeBatch(db);
    
    for (const v of reversedVehicles) {
      // Use the ID from initialData if it exists, otherwise generate from stock number
      const docId = v.id || v.stockNumber.replace(/[^a-zA-Z0-9]/g, '_');
      const vehicleData = {
        ...v,
        createdAt: serverTimestamp(),
        updatedAt: serverTimestamp()
      };
      createBatch.set(doc(db, 'vehicles', docId), vehicleData);
      if (window.logDebug) window.logDebug(`📝 Added to batch: ${v.title}`);
    }

    if (window.logDebug) window.logDebug("📤 Committing batch to cloud...");
    await withTimeoutAndRetry(() => createBatch.commit(), 'Create Batch Commit');
...
debugInfoBtn.addEventListener('click', async () => {
  ...
  try {
    const lSnap = await getDoc(doc(db, 'settings', 'landingPage'));
    if (lSnap.exists()) {
      const lData = lSnap.data();
      const secCount = lData.secondaryFeatured ? lData.secondaryFeatured.length : 0;
      window.logDebug(`🏠 Landing Config: Main=${lData.mainFeatured?.vehicleId || 'NONE'}, Sec=${secCount}`);
    } else {
      window.logDebug("🏠 Landing Config: NOT FOUND", true);
    }
  } catch (e) {
    window.logDebug(`❌ Landing Config Error: ${e.message}`, true);
  }
